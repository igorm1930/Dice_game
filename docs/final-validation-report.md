...
| Winning score configurable                             | PASS   | `createGame`; `create-game-panel.tsx`                                                           | `accepts a custom winning score`, `accepts the exact bounds`, `refuses a winning score outside the ruleset bounds`                                                                                                                                  | `MIN_WINNING_SCORE` is **2**, lowered from 10 so an e2e match could be won in one hold — a product bound moved for test convenience. The UI sets the score only at creation, not on New Game |
